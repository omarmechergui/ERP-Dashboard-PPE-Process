      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]">
        <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          <h2 className="text-lg font-bold text-slate-900">
            Historique: <span className="text-blue-600">{panneauId}</span>
          </h2>
          <button 
            onClick={onClose} 
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-white">
          {loading ? (
            ...
            <div className="text-center py-10 text-slate-500">
              ...
              <p className="text-sm">Aucun historique trouvé pour ce panneau.</p>
            </div>
          ) : (
                ...
                <div key={idx} className="relative pl-6">
                  ...
                  <div className="bg-slate-50 border border-slate-100 rounded-lg p-3 space-y-1">
                    <div className="flex justify-between items-center gap-2">
                      <p className="text-xs font-bold text-slate-900 uppercase tracking-wider">{event.action || event.etat}</p>
                      {event.previous_state && event.new_state && (
                        <div className="text-[10px] font-bold flex items-center gap-1.5 bg-blue-50 text-blue-700 px-2 py-0.5 rounded border border-blue-100">
                    ...
                    {event.notes && (
                      <p className="text-xs text-slate-600 italic">Motif: &quot;{event.notes}&quot;</p>
                    )}
                    <div className="flex items-center gap-4 pt-1 text-[11px] text-slate-500">
                      ...
                      <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5" /> {new Date(event.date || event.timestamp).toLocaleString('fr-FR')}</span>
                    </div>
                  </div>
                </div>
              ))}
